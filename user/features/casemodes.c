 ...
 *   repo, however this is implemented by @iaap with support also for
 * one-shot-shift.
 * - @sevanteri
 *     - fixed xcase waiting mode to allow more modified keys and keys from
 * other layers.
...
static void place_delimiter(void) {
  if (IS_OSM(xcase_delimiter)) {
    // apparently set_oneshot_mods() is dumb and doesn't deal with handedness
    // for you
    uint8_t mods = xcase_delimiter & 0x10 ? (xcase_delimiter & 0x0F) << 4
                                          : xcase_delimiter & 0xFF;
...
__attribute__((weak)) bool terminate_case_modes(uint16_t keycode,
...
use_default_xcase_separator(uint16_t keycode, const keyrecord_t *record) {
  // for example:
  /* switch (keycode) { */
  /*     case KC_A ... KC_Z: */
  /*     case KC_1 ... KC_0: */
    ...
    if (record->event.pressed) {
      ...
      if (xcase_state == XCASE_ON) {
        ...
        // don't increment distance to last delim if negative
          ...
          // puts back a one shot delimiter if you we're back to the delimiter
          // pos
